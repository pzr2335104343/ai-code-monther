 ...
 */
    public static File executeSaver(Object codeRequest, CodeGenTypeEnum codeGenTypeEnum) {

    // 使用switch表达式根据代码生成类型选择对应的保存方式
        return switch (codeGenTypeEnum) {
        // 当类型为HTML时，使用HTML代码文件保存器保存代码
            case HTML -> htmlCodeFileSaverTemplate.saveCode((HtmlCodeResult) codeRequest);
        // 当类型为MULTI_FILE时，使用多文件代码保存器保存代码
            case MULTI_FILE ->multiFileCodeSaverTemplate.saveCode((MultiFileCodeResult) codeRequest);
        // 默认情况，抛出非法参数异常
